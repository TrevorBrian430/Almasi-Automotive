...
export default function AdminDashboardClient({ cars }: { cars: AlmasiCar[] }) {
    /* ─── Top KPIs ─── */
    const kpiCards = [
        { label: "Total Inventory", value: cars.length.toString(), icon: Package, change: `${cars.filter(c => c.status === "Available").length} available`, positive: true },
